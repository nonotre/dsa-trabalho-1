...
as contagens medidas coincidam com as teóricas, que dependem apenas
de ⌊log₂ n⌋ + 1 (o número de iterações no pior caso). O contraste
com a busca sequencial deve ser marcante: multiplicar `n` por 1.000
(de 1.000 para 1.000.000) apenas dobra o número de iterações (de 10
para 20), e o total de operações fica na casa das dezenas mesmo para
um milhão de elementos. O tempo de execução de cada chamada é da
ordem de dezenas de nanossegundos, próximo da resolução prática do
relógio e do custo da própria chamada a `clock_gettime`; por isso a
dispersão relativa nas 100 medições tende a ser maior que nos
algoritmos lineares, e a diferença de tempo entre os cinco tamanhos
deve ser pequena, com crescimento quase imperceptível.*
